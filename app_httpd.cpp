...
static int run_face_recognition(dl_matrix3du_t *image_matrix, box_array_t *net_boxes){
    ...
    // aligned_face  == data of the current face
    if (align_face(net_boxes, image_matrix, aligned_face) == ESP_OK){ // Get Face
        if (is_enrolling == 1) { // If Enroll Face in Pressed Add this Page to the List 
            int8_t left_sample_face = enroll_face(&id_list, aligned_face); // Save The Face to the list "id_list"
            ...
            matched_id = recognize_face(&id_list, aligned_face); // See if face exists in id_list
            if (matched_id >= 0) { // Face Recognized
...
static esp_err_t stream_handler(httpd_req_t *req){
    ...
    char * part_buf[64];
    dl_matrix3du_t *image_matrix = NULL;  // Save the Image (Face)  inside (int w, int h, int c, n, stride, fptp_t *item)
    bool detected = false;
    int face_id = 0;
    int64_t fr_start = 0;
    int64_t fr_ready = 0;
            ...
            if(!detection_enabled || fb->width > 400){
                ...
                if (!image_matrix) {
                    ...
                    if(!fmt2rgb888(fb->buf, fb->len, fb->format, image_matrix->item)){
                        ...
                        if(detection_enabled){ // If Face Detection
                        ...
                        if (net_boxes || fb->format != PIXFORMAT_JPEG){
                            if(net_boxes){
                                detected = true;
                                if(recognition_enabled){ // If Face Recognition
                                    face_id = run_face_recognition(image_matrix, net_boxes);
                                }
...
static esp_err_t hostpot_handler(httpd_req_t *req) {// Get Parametres
    char username[32];//"alexandrosplatanios";
    char password[32];//"Platanios719791";
    strcpy(username, readFile(SD_MMC, "/username.txt").c_str());
    strcpy(password, readFile(SD_MMC, "/password_u.txt").c_str());
    ...
    if (uname && pass) {
        deleteFile(SD_MMC, "/run.txt");
        writeFile(SD_MMC, "/run.txt", "false");
        ESP.restart();
    }
}
...
void startCameraServer(){
    ...
    httpd_uri_t fuctor_reboot_uri = {
        .uri       = "/reboot",
        .method    = HTTP_GET,
        .handler   = fuctor_reboot_handler,
        .user_ctx  = NULL
    };
    ...
    httpd_uri_t hostpot_uri = {
        .uri       = "/hostpot",
        .method    = HTTP_GET,
        .handler   = hostpot_handler,
        .user_ctx  = NULL
    };

    httpd_uri_t stop_hostpot_uri = {
        .uri       = "/stophostpot",
        .method    = HTTP_GET,
        .handler   = stop_hostpot_handler,
        .user_ctx  = NULL
    };
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(camera_httpd, &fuctor_reboot_uri);
        httpd_register_uri_handler(camera_httpd, &restart_uri);
        httpd_register_uri_handler(camera_httpd, &hostpot_uri);
